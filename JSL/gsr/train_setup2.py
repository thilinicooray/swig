...
def main(args=None):
    ...
    #print("loading model")
    retinanet = model_setup2.resnet50(num_classes=dataset_train.num_classes(), num_nouns=dataset_train.num_nouns(), parser=parser, pretrained=True)
...
def train(retinanet, optimizer, dataloader_train, parser, epoch_num, writer):
    ...
    for iter_num, data in enumerate(dataloader_train):
        ...
        _, _, _, all_noun_loss = retinanet(image, annotations, verbs, roles, widths, heights, epoch_num, True, False)
...
def evaluate(retinanet, dataloader_val, parser, dataset_val, dataset_train, verb_orders, dev_gt, epoch_num, writer, noun_dict):
    ...
    for iter_num, data in enumerate(dataloader_val):
        ...
        with torch.no_grad():
            verb_guess, noun_predicts, _, _ = retinanet(x, annotations, y, roles, widths, heights, epoch_num, use_gt_verb=True)
