...
def main(args=None):
    ...
    retinanet = model_setup7.resnet50(num_classes=dataset_train.num_classes(), num_nouns=dataset_train.num_nouns(), parser=parser, pretrained=True)
...
def train(retinanet, optimizer, dataloader_train, parser, epoch_num, writer):
    ...
    for iter_num, data in enumerate(dataloader_train):
        ...
        class_loss, reg_loss, bbox_loss, all_noun_loss = retinanet(image, annotations, verbs, roles, adj, widths, heights, epoch_num, True, False)
...
def evaluate(retinanet, dataloader_val, parser, dataset_val, dataset_train, verb_orders, dev_gt, epoch_num, writer, noun_dict):
    ...
    for iter_num, data in enumerate(dataloader_val):
        ...
        with torch.no_grad():
            verb_guess, noun_predicts, bbox_predicts, bbox_exists = retinanet(x, annotations, y, roles, adj, widths, heights, epoch_num, use_gt_verb=True)
    ...
    eval_avg = ( evaluator.value() + evaluator.value_all() + evaluator.value_bbox() + evaluator.value_all_bbox())/ 4
